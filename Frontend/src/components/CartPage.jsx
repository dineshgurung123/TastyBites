...
const CartPage = () => {
  ...
  useEffect(() => {
    const fetchCartItems = async () => {
      try {
        const response = await axios.get("https://tastybites-2-gcm4.onrender.com/api/carts", {
  ...
  const handleRemoveItem = async (cartId) => {
    try {
      await axios.delete(`https://tastybites-2-gcm4.onrender.com/api/carts/${cartId}`, {
        withCredentials: true,
      });
  ...
  const handleUpdateQuantity = async (itemId, newQuantity, foodId) => {
    try {
      ...
      await axios.put(
        "https://tastybites-2-gcm4.onrender.com/api/carts",
  ...
  const handlePlaceOrder = async () => {
    try {
      const response = await axios.post(
        "https://tastybites-2-gcm4.onrender.com/api/orders",
        {},
        {
          withCredentials: true,
        }
